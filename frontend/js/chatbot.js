...
document.querySelector('.send-button').addEventListener('click', async function() {
  ...
  const userEmail = document.getElementById('email-input').value; // Asegúrate de tener un campo para el correo

  if (!userEmail.trim()) {
    alert("Por favor, introduce tu correo antes de enviar un mensaje.");
    return;
  }
  ...
  if (userInput.trim() !== "") {
    ...
    // Respuesta del bot
    const botResponse = await getBotResponse(userInput, userEmail);
...
async function getBotResponse(userMessage) {
  try {
    const response = await fetch(BASE_URL + "/chatbot/send-message", {
        ...
        body: JSON.stringify({ message: userMessage, id_usuario: 1, correo: "juan.perez@example.com" })
    });
